...
# functions
def getStatus(workspaceId : str):
    '''
    Works same as git status command in git.
    ...
    Gives workspaceHead for the particular workspace, which will be used to commit, updates changes

    params : str workspaceId -> workspace id of Fabric workspace
    '''
    url = f'https://api.fabric.microsoft.com/v1/workspaces/{workspaceId}/git/status'

    response = requests.get(url=url, headers=headers)
    print(response.status_code)
    
    # changes 
    print(response.json()['changes'])

    if response.status_code >= 400 :
        print(response.json()['errorCode'], response.json()['message'])

getStatus('9f06650a-4b74-42c4-bf0d-9cd7d5d6b140')
